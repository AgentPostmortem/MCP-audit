...
```bash
npx mcp-audit stdio "node my-mcp-server.js"
```
...
npx mcp-audit http https://mcp.example.com/mcp --token "$MCP_TOKEN"

# Lint a server's declared surface from a manifest, without running it
npx mcp-audit static ./mcp-manifest.json
...
```yaml
- run: npx mcp-audit static ./mcp-manifest.json --sarif --output mcp-audit.sarif --fail-on critical
